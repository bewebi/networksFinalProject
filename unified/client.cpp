...
int custom_read(int fd, void * buffer, int nbytes); /* Shawyoun fork */
int custom_write(int fd, void * buffer, int nbytes); /* Shawyoun fork */
...
int main(int argc, char *argv[]) {
    if(argc != 3 && argc != 4) {
    fprintf(stdout, "Usage: ./client <host> <port> [-s]\n");
    return 1;
    }
    fprintf(stdout, "Welcome to Bernie and Shawyoun's final project!\n\n");

	if(argc == 4 && strcmp(argv[3], "-s") == 0){ /* Shawyoun forked */
		secureMode = 1;

		char ch = '\0'; int resultForTLS = 0;
	    	fprintf(stdout, "Starting client in secure mode. What version of TLS would you like to use? (Enter 0 for TLS 1.0, 1 for TLS 1.1, 2 for TLS 1.2. All other values: highest possible version is used).\n");
	    	while(read(0,&ch,1) > 0) {
		       if(ch == 10) {
			   break;
	    	    } else {
			   	resultForTLS = atoi(&ch);
			} 
	    	}
	    	if(resultForTLS == 0 || resultForTLS == 1 || resultForTLS == 2) {
			ctx = newSSLContext(resultForTLS);
	    	}else{
			ctx = newSSLContext(3);
		}

		fprintf(stdout, "If the certificate presented is a self-signed certificate, and thus we cannot verify its identity, should we still proceed with encrypted connection? (Enter 'y' for yes, any other input for no.)\n");
	...
	if(secureMode){ /* Shawyoun forked */
		ssl = SSL_new(ctx); 
		SSL_set_fd(ssl, sockfd);
		if (SSL_connect(ssl) != 1){
			ERR_print_errors_fp(stderr);
			fprintf(stderr, "Problem establishing SSL connection. Quitting.\n");
			exit(1);
		}else{
			fprintf(stdout, "SSL Handshake successful!\n\n");
		}
	}
...
void sendMessage() {
    ...
    do {
        bytes = custom_write(sockfd,(char *)&headerToSend+sent,total-sent);
    ...
    if(i > 1) {
        ...
        sent = 0;
        while(sent < total) {
            bytes = custom_write(sockfd, buffer+sent, total-sent);
            if(bytes < 0) error("ERROR writing message to socket");
            if(bytes == 0) break;
            sent+= bytes;
...
void readMessage() {
    ...
    while(received < total) {
        bytes = custom_read(sockfd,headerBuffer+received,total-received);
    ...
    if(headerToRead.length > 0) {
        ...
        received = 0;
        while(received < total) {
            bytes = custom_read(sockfd,dataBuffer+received,total-received);
            ...
            for(int i = 0; i < FD_SETSIZE; i++) {
                if(FD_ISSET(i, &stdin_set)) {
                    if(i == 1) { // iput on stdin
                        ...
                        do {
                            bytes = custom_write(sockfd,(char *)&draftResponse+sent,total-sent);
                            if(bytes < 0) error("ERROR writing message to socket");
                            if(bytes == 0) break;
                            sent+=bytes;
                        } while (sent < total);
                        ...
                        sent = 0;
                        while(sent < total) {
                            bytes = custom_write(sockfd, dataBuffer+sent, total-sent);
...
void sendHello() {
    ...
    usleep(sleepTime); // Simulate latency
    do {
        bytes = custom_write(sockfd,(char *)&helloHeader+sent,total-sent);
        if(bytes < 0) error("ERROR writing message to socket");
        if(bytes == 0) break;
        sent+=bytes;
    ...
    sent = 0;
    while(sent < total) {
        bytes = custom_write(sockfd, pword+sent, total-sent);
        if(bytes < 0) error("ERROR writing password to socket");
        if(bytes == 0) break;
        sent+= bytes;
...
void sendPlayerRequest(bool quiet) {
    ...
    usleep(sleepTime); // Simulate latency
    do {
        bytes = custom_write(sockfd,(char *)&playerReqHeader+sent,total-sent);
        if(bytes < 0) error("ERROR writing message to socket");
        if(bytes == 0) break;
        sent+=bytes;
        //fprintf(stdout,"Sent %d bytes of the hello header\n",sent);
...
void sendStartDraft() {
    ...
    usleep(sleepTime); // Simulate latency
    do {
        bytes = custom_write(sockfd,(char *)&startDraftHeader+sent,total-sent);
        if(bytes < 0) error("ERROR writing message to socket");
        if(bytes == 0) break;
        sent+=bytes;
...
void sendExit() {
    ...
    usleep(sleepTime); // Simulate latency
    do {
        bytes = custom_write(sockfd,(char *)&exitHeader+sent,total-sent);
        if(bytes < 0) error("ERROR writing message to socket");
        if(bytes == 0) break;
        sent+=bytes;
        //fprintf(stdout,"Sent %d bytes of the exit header\n",sent);
...
void replyPing(int pingID) {
    ...
    do {
        bytes = custom_write(sockfd,(char *)&pingReplyHeader+sent,total-sent);
        if(bytes < 0) error("ERROR writing message to socket");
        if(bytes == 0) break;
        sent+=bytes;
       //fprintf(stdout,"Sent %d bytes of the header\n",sent);
    ...
    sent = 0;
    while(sent < total) {
        bytes = custom_write(sockfd, timeBuffer+sent, total-sent);
        if(bytes < 0) error("ERROR writing message to socket");
        if(bytes == 0) break;
        sent+= bytes;
        //fprintf(stdout,"Sent %d bytes of the message body\n")
...
int custom_write(int fd, void * buffer, int nbytes){
	if(secureMode)
		return SSL_write(ssl, buffer, nbytes);
	else
