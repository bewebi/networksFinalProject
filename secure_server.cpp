...
fd_set active_fd_set, read_fd_set;
    SSL_CTX * ctx; // Declared here so all functions can use
...
void handleHello(struct clientInfo *curClient) {
    ...
    if(draftStarted && curClient->readyToDraft) {
		...
		sent = 0;
		while(sent < total) {
	        bytes = SSL_write(curClient->ssl, stringBuffer+sent, total-sent);
	        if(bytes < 0) error("ERROR writing to socket");
	        if(bytes == 0) break;
	        sent+= bytes;
	    }
    ...
    for(int i = 0; i < MAXCLIENTS; i++) {
    	if(clients[i].active && (clients[i].sock != curClient->sock)) {
		    ...
		    do {
				bytes = SSL_write(clients[i].ssl, (char *)&responseHeader+sent, total-sent);
				if(bytes < 0) error("ERROR writing to socket");
				if(bytes == 0) break;
				sent+=bytes;
		    } while (sent < total);
			...
			sent = 0;
			while(sent < total) {
	        	bytes = SSL_write(clients[i].ssl, stringBuffer+sent, total-sent);
		        if(bytes < 0) error("ERROR writing to socket");
		        if(bytes == 0) break;
	    	    sent+= bytes;
		    }
...
void draftNewRound() {
    ...
    for(int i = 0; i < MAXCLIENTS; i++) {
    	if(clients[i].active) {
		    ...
		    sent = 0;
		    while(sent < total) {
		        bytes = SSL_write(clients[i].ssl, curPlayer+sent, total-sent);
		        if(bytes < 0) error("ERROR writing to socket");
		        if(bytes == 0) break;
		        sent+= bytes;
		    }
...
void endDraftRound() {
	...
	for(int i = 0; i < MAXCLIENTS; i++) {
		if(clients[i].active) {
		    ...
		    sent = 0;
		    while(sent < total) {
		        bytes = SSL_write(clients[i].ssl, winner+sent, total-sent);
		        if(bytes < 0) error("ERROR writing to socket");
		        if(bytes == 0) break;
		        sent+= bytes;
		    }			
